...
# Dimension de Tipo de acopio
dim_tipo_acopio_source = os.path.join(current_path, 'queries','dim_tipo_acopio_source.sql')
with open(dim_tipo_acopio_source) as fichero:
    Q_TIPO_ACOPIO_SOURCE = yaml.safe_load(fichero)

dim_tipo_acopio_target = os.path.join(current_path, 'queries','dim_tipo_acopio_target.sql')
with open(dim_tipo_acopio_target) as fichero:
    Q_TIPO_ACOPIO_TARGET = yaml.safe_load(fichero)
    

# Dimension de vigencia zona
dim_vigencia_zona_source = os.path.join(current_path, 'queries','dim_vigencia_zona_source.sql')
with open(dim_vigencia_zona_source) as fichero:
    Q_VIGENCIA_ZONA_SOURCE = yaml.safe_load(fichero)

dim_vigencia_zona_target = os.path.join(current_path, 'queries','dim_vigencia_zona_target.sql')
with open(dim_vigencia_zona_target) as fichero:
    Q_VIGENCIA_ZONA_TARGET = yaml.safe_load(fichero)
    

# Dimension de vigencia zona
dim_inventariable_source = os.path.join(current_path, 'queries','dim_inventariable_source.sql')
with open(dim_inventariable_source) as fichero:
    Q_INVENTARIABLE_SOURCE = yaml.safe_load(fichero)

dim_inventariable_target = os.path.join(current_path, 'queries','dim_inventariable_target.sql')
with open(dim_inventariable_target) as fichero:
    Q_INVENTARIABLE_TARGET = yaml.safe_load(fichero)
